...
//TODO 멘토님 저 질문있어요 !!!!!
//1. 레코드로 선언된 DTO는 서비스 단으로 인자가 넘겨져도 괜찮은 건가요? (31번 라인, 59번 라인) feedback: 절대~ 하지마
//2. 레코드를 사용하기 위해 Controller에 Entity 코드가 import 되어도 되는 건가요..? (5번 라인) // feedback: 절대~ 하지마

import java.time.LocalDateTime;
import java.util.List;

@RestController
@RequestMapping("/todo") // 복수형으로 하라고요!!
...
public class TodoController {
    ...
    @GetMapping("/{todoId}")
    TodoResponse getTodo(@PathVariable("todoId") long todoId){ // @PathVariable("todoId") >> @PathVariable 로 바꾸기
        return todoService.getTodo(todoId);
    }

    @PatchMapping("/complete/{todoId}")
    void todoDone(@PathVariable("todoId") long todoId){
        todoService.todoComplete(todoId);
    }

    @PatchMapping("/setback/{todoId}")
    void todoWithDraw(@PathVariable("todoId") long todoId){
        todoService.todoSetBack(todoId);
    }

    @PatchMapping("/{todoId}")
    TodoResponse updateTodo(@PathVariable("todoId") long todoId, @RequestBody TodoDto todoDto){
